...
def plot_distribution0(df,client_id,vars,nrow,ncol):
    colors = ['red', 'green']
    i = 0
    ...
    for feature in vars:
        ...
        plt.axvline(client, c='orange', linewidth=2,  alpha=0.8,label='Client_position')
        plt.legend()
...
def plot_frequency0(df, client_id, vars_, nrow, ncol):
    colors = [ '#32CD32', 'red']
    sns.set_style('whitegrid')
    fig, axes = plt.subplots(nrow, ncol, figsize=(12, 5 * nrow))
    axes = axes.flatten()  # Aplatir la grille d'axes pour faciliter l'indexation
